...
11. **Disciplina de EOL al propagar a satélites.** Los documentos satélite tienen EOL fijo por archivo: `DECISIONES_NO_REABRIR.md` y `Pendiente_pre_produccion.md` = **LF**; `ESTADO_ACTUAL_VITA_DELTA.md` = **CRLF puro**; `Lecciones_Aprendidas.md` = **mixto (LF+CRLF)**. Al editarlos, **verificar y preservar el EOL real** de cada uno (no convertir), con anclas únicas (`count==1`) y sin mega-diffs.
