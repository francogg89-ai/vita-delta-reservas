...
- **Validado en TEST:** batería de 9 smokes + pago parcial + **rollback multi-línea** (0 pagos tras abort, verificado por `source_event`). Recargo correcto en los tres casos con transferencia ($8.500 / $5.000 / $1.650). **No promovido a OPS** (promoción diferida con aprobación explícita).
...
Con DEV, TEST, OPS, 6D, 7A-7E, **8A-8D cerradas y la Etapa 8 (operación real interna) completa**, la **sub-etapa 8C-bis** cerrada y activa en OPS, y la **Etapa 9 / Carril A** en marcha (9A diagnóstico + 3a-v2 listado + **9B/3b cobranza posterior cerrada y validada en TEST**), el equipo opera el complejo con tres acciones autoservicio sobre OPS y recibe avisos automáticos por mail. Las opciones a priorizar por Franco de acá en más:

- **Promover 9B / Fase 3b a OPS:** crear primero `public.abortar_si_falla(jsonb)` en OPS (aditiva; si falta, 3b falla en runtime), luego importar el workflow `__OPS` (credencial `vita_supabase_ops`, Basic Auth propia, marcador de entorno) y hacer smoke con datos reales. TEST antes que OPS. Ver `Pendiente_pre_produccion.md`.
- **Etapa 9 / Carril B — políticas de liquidación:** split operativo 75/25, reparto entre socios, y liquidación del `extra` (¿repartible? ¿gasto financiero? ¿comisión? ¿ingreso separado?). Abierto, sin iniciar.
- **Arquitectura global de contabilidad (schema futuro):** gastos, caja por lugar, conversión/tabla de ahorro de monedas, cancelaciones con cargo, AFIP/ARCA/IVA. Implicará nueva versión del canónico y su propia ronda de satélites. Conversación aparte.
