...
3. Docs/Implementacion/6B_SCHEMA_SQL.md (schema canónico actual: **v1.11.0**)
...
- **Próximo frente backend contable — snapshot mensual / foto congelada + L3 histórico (prioridad actual):** la foto mes-a-mes de la cuenta corriente (habilita la lectura L3; hoy diferida por D-CC-11). El **bootstrap kit NO se regenera** hasta cerrar este frente (deuda **P-CC-4**: hoy pineado en v1.9.0, rezagado respecto de v1.11.0). La **UI del retiro** en el portal (botón "Retirar saldo") queda **para después del backend contable** — el backend/gateway del retiro ya está en OPS. El resto (Carril C ya promovido, **Mercado Pago**, **web pública**, **bot conversacional**) son **frentes posteriores**, no el siguiente paso principal.
