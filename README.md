...
### Contabilidad operativa interna — Carril B (Etapas 9C-9E)
...
Pendiente del Carril B (no iniciado): **9F** (gasto rediseñado con clases A/C/D/E, alcance por zona y pagador-socio — primera sub-etapa que vuelve a escribir), **9G** (cascada read-only de liquidación, donde se define el **% operativo** con los socios) y **9H** (cuenta corriente interna de socios: saldos acumulados, retiros y conversión ARS→USD — capa **posterior con estado**, encuadrada en `PENDIENTE_CARRIL_B_9H_SALDOS_RETIROS_REVALUACION.md`, **sin diseñar todavía**). El **% operativo** de la cascada sigue **abierto** (a definir entre socios); la cascada no se congela hasta entonces.
...
Con DEV, TEST, OPS, 6C, 6D, 7A-7E, **8A-8D cerradas y la Etapa 8 (operación real interna) completa**, la **sub-etapa 8C-bis** cerrada y activa en OPS, y la **Etapa 9 en marcha** (Carril A: diagnóstico 9A + listado de saldos + cobranza posterior 9B/3b validada en TEST; **Carril B: 9C-9E cerradas y verificadas en TEST**), el sistema está **operativo**: el equipo carga reservas, ve el estado en los calendarios y crea bloqueos (autoservicio sobre OPS), y recibe avisos automáticos por mail. Las opciones a priorizar de acá en más (orden sugerido, no comprometidas):
...
- **Contabilidad — Carril B (en TEST):** las sub-etapas **9C** (catálogo/zonas/titularidad), **9D** (activación por rango) y **9E** (matriz dinámica + reparto) están **cerradas y verificadas en TEST**. Siguen, en orden: **9F** (gasto rediseñado con clases A/C/D/E, alcance por zona y pagador-socio — primera sub-etapa del Carril B que vuelve a escribir), **9G** (cascada read-only de liquidación, donde se cierra el **% operativo** con los socios) y **9H** (cuenta corriente interna: saldos, retiros y conversión ARS→USD — capa posterior con estado, encuadrada en su breadcrumb, sin diseñar). La promoción de todo el Carril B a OPS es una **operación coordinada única**, con bump único del canónico y siembra de `abortar_si_falla` + marcador de entorno en OPS.
...
- Contabilidad automatizada completa. *Avance del **Carril B** en TEST: catálogo, activación y matriz de reparto (9C-9E) construidos y verificados; faltan el gasto rediseñado (9F), la cascada de liquidación (9G) y la cuenta corriente de socios con conversión de monedas (9H, encuadrada pero sin diseñar). La cobranza posterior 9B/3b (Carril A) es el primer ladrillo que escribe pagos, validado en TEST. **Nada de esto está en OPS ni en el canónico todavía.*** La facturación fiscal (AFIP/ARCA/IVA) queda fuera del Carril B.
