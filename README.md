...
- **Carril C — Portal Operativo Interno (diseño de Backend/API CERRADO; construcción sin empezar):** hub único con una sola autenticación (Supabase Auth), gateway server-side (Edge Function `portal-api`) que firma HMAC a n8n, y menú por rol (socio amplio; Vicky reservas + lecturas operativas; Jenny limpieza) con **permisos reales** y datos sensibles ocultos por rol. La etapa de **Backend/API vía n8n** quedó **cerrada en diseño** (catálogo de 25 acciones A01–A25, matriz rol×endpoint, modelo de identidad/seguridad, MVP por slices, estrategia de pruebas; **D-C-01…28**). **Nada construido, OPS y schema intactos** (canónico v1.8.0 sin bump). Carril **independiente del Carril B**. Cierre: `Docs/Bitacora/CarrilC/CARRIL_C_BACKEND_API_DISENO_CIERRE.md`. **Próximo:** construcción de **Slice 0** (espina de seguridad) sobre TEST.
