...
**Al 2026-06-14:** el **Carril B (contabilidad operativa interna entre socios) está promovido a OPS** y el schema canónico quedó en **v1.8.0**. OPS opera reservas reales (8B), calendarios visuales (8C), bloqueos (8D) y avisos por mail (8C-bis), y ahora también aloja la estructura del Carril B (cuenta corriente interna). **DEV quedó fuera del alcance de la promoción** y se reconstruirá desde v1.8.0.
...
|---|---|---|
| DEV | `vita-delta-dev` | Backend v1.7.3 + hardening + 8 workflows validados + permisos EXECUTE endurecidos (7E). IDs de cabaña 17-21. **Fuera del alcance de la promoción del Carril B: queda atrás y se reconstruirá desde el canónico v1.8.0** (cuenta Supabase aparte). |
...
- **Reconstrucción de DEV desde v1.8.0** — DEV quedó fuera del alcance de la promoción (cuenta Supabase aparte); se rearma de cero desde el canónico v1.8.0 (autocontenido, Parte B + Parte C) en una etapa posterior. No bloquea OPS. `Pendiente_pre_produccion.md`, sección reconstrucción de DEV.
...
- `Docs/Implementacion/6B_SCHEMA_SQL.md` — schema canónico SQL vigente (**v1.8.0**, incorpora el Carril B; ver `Docs/Bitacora/Promocion/PROMOCION_CARRIL_B_OPS_CIERRE.md`).
