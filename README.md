...
Ubicación típica: `Workflows/n8n/*.template.json` (directamente, sin subcarpeta `Supabase/`).
...
- **Carril C — Portal Operativo Interno (diseño de Backend/API + Slice 0 + Slice 1 cerrados sobre TEST):** hub único con una sola autenticación (Supabase Auth), gateway server-side (Edge Function `portal-api`) que firma HMAC a n8n, y menú por rol (socio amplio; Vicky reservas + lecturas operativas; Jenny limpieza) con **permisos reales** y datos sensibles ocultos por rol. Cerrados: el **diseño de Backend/API** (catálogo de 25 acciones A01–A25, matriz rol×endpoint, modelo de identidad/seguridad, MVP por slices; D-C-01…28), el **Slice 0** (espina de seguridad: `portal_usuarios` + `portal-api` + revalidación HMAC en n8n; D-C-29…35) y el **Slice 1** (las 5 lecturas operativas A03/A04/A05/A06/A12 vía wrappers n8n firmados; D-C-36…49 / L-C-10…15). **OPS y schema intactos** (canónico **v1.8.1**; el Carril C es TEST-only, no entra al canónico hasta una promoción coordinada). Carril **independiente del Carril B**. Cierre: `Docs/Bitacora/CarrilC/C_SLICE1_CIERRE.md`. **Próximo:** Slice 2 (escrituras) sobre TEST.
...
- `Workflows/n8n/*.template.json` — los 7 workflows **legacy** contra Google Sheets (`db_*`), congelados como referencia histórica.
