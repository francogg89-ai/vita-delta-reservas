...
**Al 2026-06-26:** el **Carril B (contabilidad operativa interna entre socios) está promovido a OPS** y el schema canónico quedó en **v1.8.1** (v1.8.0 consolidó el Carril B; **v1.8.1 canoniza el hardening de las funciones base del motor**, Bloque 23). OPS opera reservas reales (8B), calendarios visuales (8C), bloqueos (8D) y avisos por mail (8C-bis), y aloja la estructura del Carril B (cuenta corriente interna). **DEV fue reconstruido desde cero** en un proyecto Supabase nuevo (cerrado como OPS, `ambiente='dev'`, IDs 1-5, Carril B completo): **los tres entornos DEV/TEST/OPS quedan alineados** sobre el mismo canónico. En paralelo, el **Carril C — Portal Operativo Interno** pasó de diseño a construcción: cerró el **diseño de Backend/API** (2026-06-15) y luego el **Slice 0 (espina de seguridad)** sobre **TEST** (2026-06-16) — Supabase Auth + tabla `portal_usuarios` interna + Edge Function `portal-api` (gateway/BFF, JWT→rol→allowlist→HMAC) + revalidación en n8n (HMAC sobre raw body + `ts` + `ambiente`), validado **6/6 + 4/4**; **OPS y schema (v1.8.1) intactos** (`portal_usuarios` es TEST-only). Sobre eso cerró el **Slice 1 (lecturas reuse)** (2026-06-18): las cinco primeras lecturas operativas (A03/A04/A05/A06/A12) vía wrappers n8n firmados, con el gateway imponiendo allowlist doble + action binding (CATALOG de 6 acciones: 1 Edge + 5 n8n), validado por bloque (**D-C-36…49 / L-C-10…15**); **OPS y schema (v1.8.1) siguen intactos** (lecturas read-only; `portal_usuarios` TEST-only). Sobre eso cerró el **Slice 2 (escrituras reuse)** (2026-06-20): las **tres primeras escrituras operativas** — A07 `reserva.crear_manual` (reusa `crear_prereserva`/`registrar_pago`/`confirmar_reserva` de 8B), A08 `bloqueo.crear_manual` (reusa `crear_bloqueo` de 8D) y A10 `cobranza.registrar_saldo` (reusa `registrar_pago`/`abortar_si_falla` de 9B) — vía wrappers n8n firmados que **inyectan el actor server-side** desde el JWT (nunca del payload) y, por ser escrituras, devuelven `estado_incierto` ante dispatch no confiable; A10 suma escritura transaccional con **advisory lock** por `id_reserva` + idempotencia de match exacto por `idempotency_key` (D-C-50/51; idempotencia fuerte, distinta de la débil D-9B-07 del W09 viejo). Gateway con CATALOG de **9 acciones** (1 Edge + 5 lecturas + 3 escrituras), validado por capa (directo + gateway) con el assert de regresión SOBREPAGO → `conflicto` corrido end-to-end (**D-C-50/51 / L-C-16/17**); **OPS y schema (v1.8.1) siguen intactos** (las tres escrituras reusan funciones del motor; `portal_usuarios` TEST-only). Sobre eso cerró el **Slice 3a (lecturas nuevas)** (2026-06-20): las **dos primeras lecturas nuevas** — A24 `historico.reservas` (buscador operativo de reservas: floor duro 2026-07-01, `saldo_real` por la CTE de A12 desde `reservas.id_pre_reserva`, devuelve `id_cabana`+`cabana`) y A25 `ingresos.cobrados_periodo` (caja percibida: solo `sena`+`saldo` confirmados, suma sobre `pagos`, `otros_movimientos` informativo no sumado, `periodo_hasta` híbrido) — escritas como `SELECT` inline (sin funciones nuevas ni DDL), **sin** `injectActor` ni `isWrite`; el gateway **preserva la ausencia de `periodo_hasta`** para no cambiar la semántica directo↔gateway. CATALOG de **11 acciones** (1 Edge + 7 lecturas + 3 escrituras), validado por capa cruzando el snapshot al centavo (A24 26/26 + 23/23; A25 26/26 + 24/24; **D-C-52…54 / L-C-18/19**); **OPS y schema (v1.8.1) siguen intactos** (A24/A25 son `SELECT` inline; `portal_usuarios` TEST-only). Sobre eso cerró el **Slice 3b (gastos: A11 carga + A13 listado)** (2026-06-22): el **gasto interno** del portal — A11 `cargar.gasto_interno`, la **primera escritura no-idempotente** del portal, con guard de **dos capas en UNA transacción** (anti-replay `UNIQUE(nonce)` con `nonce` derivado de la firma HMAC + idempotencia de negocio `UNIQUE(action,idempotency_key)` comparando `payload_norm`+`actor`) dentro de `portal_cargar_gasto_interno`, y A13 `gastos.listado`, lectura companion por período contable (`SELECT` inline, bordes truncados a primer día de mes, gateway que espeja la semántica mensual) — cada una por wrapper n8n directo **y** por gateway. La infra de idempotencia (`portal_idempotencia` + `portal_cargar_gasto_interno`) es **TEST-only, fuera del canónico** (precedente `portal_usuarios`); `gastos_internos` se usa tal cual, sin DDL. CATALOG de **13 acciones** (1 Edge + 8 lecturas + 4 escrituras), validado por capa los cuatro caminos con cruce al centavo (A11 38/38 + 24/24; A13 32/32 + 34/34; paridad ejecutable gateway↔wrapper 24/24; **D-C-55…60 / L-C-20…23**); **cumple P-C-9 en TEST**; **OPS y schema (v1.8.1) siguen intactos**. Sobre eso se aprobó el **Contrato Frontend ↔ Portal v1** (2026-06-22): la API reference del portal vista desde el navegador, contra el gateway real (CATALOG 13), con reglas estructurales (transporte dual de `idempotency_key`, calendarios HTML temporal, ramificación por `body.ok` sin 401) y decisiones D-FE-01…08, sin tocar el backend (`Docs/Implementacion/Carril_C/Portal_Frontend/CONTRATO_FRONTEND_PORTAL_v1.md`). Sobre eso arrancó el **Frontend TEST**: el **sub-slice 0 (shell)** —auth + `sesion.contexto` + menú por rol desde `acciones`— quedó **validado en TEST** (2026-06-23) con evidencia visual de menú por rol (jenny solo limpieza; vicky/socio completo), sin tocar backend/n8n/OPS/canónico (**D-FE-09/10/11 / L-FE-01**). Sobre eso cerró el **sub-slice 1 (las 8 lecturas)** (2026-06-24): las 8 pantallas de lectura del portal (A03/A04 calendarios en `<iframe srcDoc>` sandbox sin scripts, A05/A06/A12, y A24/A25/A13 con filtros nativos + paginación server-side) sobre el shell, con router `react-router-dom`, hook `useAction`, `DataTable`/`Paginador` y `Money` que **nunca divide por 100**, validadas por rol en TEST (jenny solo el calendario de limpieza; vicky/socios todo). Durante la integración se aplicó un **parche de consistencia A04/A12** en el backend n8n (A04 recomputa `saldo_real` con el criterio de A12; display clampeado a `$0`; notas operativas; **D-C-61/62/63**) y se fijaron **D-FE-12…18 / L-FE-02** (`FRONTEND_SUBSLICE1_CIERRE.md`). Sobre eso cerró el **sub-slice 2 (las 4 escrituras)** (2026-06-25): A07/A08/A11 + **A10-MP `cobranza.registrar_cobro`** (cobranza multi-porción efectivo/transferencia/otros + recargo 5%, **bloqueo de sobrepago** en UI + backend en duro), con **W10 `cobranza.registrar_saldo` deprecated-in-place** y oculto del frontend por tolerancia-forward; patrón `useEnviar` + idempotencia por intento de submit + `estado_incierto` + fix de StrictMode; **D-FE-19…28 / L-FE-03…07** (`FRONTEND_SUBSLICE2_CIERRE.md`). Sobre eso cerró el **sub-slice 3 (publicación TEST + piloto operativo)** (2026-06-26): el portal se **publicó en un link TEST real** en **Vercel** (build estático + fallback SPA por `vercel.json`) con un **banner de ambiente** fijo "AMBIENTE DE PRUEBA · TEST" (derivado de `VITE_SUPABASE_URL`, ref TEST; estado defensivo para lo no reconocido; sin `VITE_AMBIENTE`) y se corrió un **piloto operativo** con los usuarios reales por rol; **7 gates en verde** y **piloto OK** (operaron sin asistencia, cero blockers, sin fixes menores); **D-FE-29/30 / L-FE-08/09**, único pendiente nuevo **P-FE-09** (extender el banner al ref OPS); sin tocar backend/n8n/OPS/canónico (`FRONTEND_SUBSLICE3_CIERRE.md`). Cerrado el frontend TEST, se **enganchó el aviso 8C-bis al alta por el portal** (A07): una **rama lateral no bloqueante** en el wrapper que dispara el mismo aviso por mail que ya disparaba 8B, reusando el sub-workflow sin tocar backend, gateway ni OPS, validado end-to-end en TEST (5 gates verdes, incluido el de no-afectación; D-C-71…73 / L-C-24/25; `AVISO_8CBIS_PORTAL_A07_CIERRE.md`). **Próximo:** **preparar la promoción coordinada del Carril C a OPS** (catálogo real P-FE-01, CORS al origin real P-C-7, GRANTs/seed real, banner OPS P-FE-09).
...
**Schema canónico actual:** `6B_SCHEMA_SQL.md v1.8.1` — incorpora el **Carril B** como capa real (sección 24 conceptual + PARTE C ejecutable), promovido a OPS el 2026-06-14. **TEST y OPS tienen el Carril B vivo**, con **paridad estructural certificada por huella** (`TOTAL_CARRIL`, 31 objetos, idéntica entre ambos); **DEV fue reconstruido desde el canónico v1.8.1** en un proyecto Supabase nuevo (cerrado como OPS, `ambiente='dev'`, IDs 1-5), paritario con TEST/OPS — **los tres entornos quedan alineados**. El núcleo previo sigue vigente: TEST se reconstruyó desde el canónico en 7B (paridad 10/10 vs DEV) y OPS en 8A (paridad P01-P10 10/10); 8B/8C/8D/8C-bis no tocaron el schema (capa de carga sobre funciones existentes, calendarios de solo lectura, bloqueos vía `crear_bloqueo()`, alerta de solo lectura + mail). Con la promoción, los objetos del Carril B —columnas `valor_relativo` e `id_socio_beneficiario` en `cabanas`; tablas `zonas`, `cabana_zona`, `activaciones_operativas`, `gastos_internos` y las cinco con estado de 9H (`liquidaciones_periodo`, `liquidacion_cascada`, `liquidacion_socio`, `movimientos_socio`, `revaluaciones`) con sus diez triggers de inmutabilidad; el marcador `'ambiente'='ops'`; y **veintiuna funciones** (las diecinueve de negocio —`resolver_beneficiario`, tres de matriz/reparto de 9E, seis read-only de la cascada de 9G y nueve de la cuenta corriente de 9H— más la función de trigger `trg_9h_inmutable` y el helper `abortar_si_falla(jsonb)` de 9B)— viven ahora en TEST, OPS y el **canónico v1.8.0**, **cerrados al Data API** (sin grants ni RLS). La `gastos` legacy quedó **conservada/congelada** (D-9F-01); el Carril B opera sobre `gastos_internos`.
...
Pendientes activos vigentes al cierre de la promoción del Carril B (2026-06-14):
...
- Contabilidad productiva con **carga real en OPS** y capa fiscal/legal completa. *El **Carril B** está **completo y promovido a OPS (9C-9H, canónico v1.8.0)**: catálogo, activación, matriz, gasto rediseñado, **cascada de liquidación validada al centavo (9G)** y **cuenta corriente interna de socios con saldos acumulados, retiros y conversión ARS→USD (9H)**. Las reglas de negocio quedaron cerradas: **% operativo = 25%** e **inicio contable = 2026-07-01** (D-NEG-01/02). La estructura ya vive en OPS (tablas arrancando vacías); **lo que falta es la carga operativa real a partir de julio 2026**. La cobranza posterior 9B/3b (Carril A) —primer ladrillo que escribe pagos— también quedó promovida.* La facturación fiscal (AFIP/ARCA/IVA) queda fuera del Carril B.
...
- `Docs/Implementacion/6B_SCHEMA_SQL.md` — schema canónico SQL vigente (**v1.8.1**: v1.8.0 incorporó el Carril B, v1.8.1 canoniza el hardening del motor en Bloque 23; ver `Docs/Bitacora/Promocion/PROMOCION_CARRIL_B_OPS_CIERRE.md`).
...
- `Docs/Implementacion/Bootstrap_Entorno_Nuevo_v1.8.1/` — juego **repetible** para levantar un entorno de cero desde el canónico **v1.8.1**: `00_PRECHECK` → `01_BOOTSTRAP_PARTE_B_BASE` → `01_VERIFY` → `02_BOOTSTRAP_PARTE_C_CARRIL_B` → `02_VERIFY_FINAL_ENTORNO`, con `README_EJECUCION_BOOTSTRAP.md`. Extracción **literal** del canónico, **fijada a v1.8.1** (regenerar en cada bump para no divergir de la paridad); validada de punta a punta contra un PostgreSQL limpio (aún **no ejecutada sobre un Supabase real**). Formaliza y generaliza la reconstrucción de DEV como herramienta de **réplica/recuperación de entornos**. Solo sobre base vacía; no correr sobre un entorno poblado (el precheck lo gatea).
