...
Con 9H **el Carril B queda completo en TEST**. Quedan servidas dos decisiones de negocio para los socios: el **valor real del % operativo** (la cascada lo recibe por parámetro; 0,25 fue valor de trabajo, sin carácter normativo) y la **política de arranque** para lo percibido con pool vacío (junio 2026). El próximo hito del carril ya no es de diseño sino de despliegue: la **promoción coordinada de todo el Carril B a OPS** (un único DDL, con el bump del canónico, sin copiar datos de TEST).
...
Pendientes activos vigentes al cierre de la Etapa 9H:

- **Promoción coordinada del Carril B a OPS (incluye 9B / 3b)** — paquete único por DDL: objetos 9C/9D/9E/9F/9G + la **capa con estado de 9H** (5 tablas + 10 triggers + 9 funciones) + `public.abortar_si_falla(jsonb)` + el workflow de cobranza apuntando a OPS, con **bump único del canónico**, marcador `'ambiente'='ops'`, GRANTs/RLS a decidir y prueba con datos reales. **Los fixtures de laboratorio no viajan** (se recrea estructura, no se copian datos). `Pendiente_pre_produccion.md`, sección de promoción coordinada.
...
Con DEV, TEST, OPS, 6C, 6D, 7A-7E, **8A-8D cerradas y la Etapa 8 (operación real interna) completa**, la **sub-etapa 8C-bis** cerrada y activa en OPS, y la **Etapa 9 en marcha** (Carril A: diagnóstico 9A + listado de saldos + cobranza posterior 9B/3b validada en TEST; **Carril B completo: 9C-9H cerradas y verificadas en TEST** (capa derivada con cascada validada 40/40 + capa con estado)), el sistema está **operativo**: el equipo carga reservas, ve el estado en los calendarios y crea bloqueos (autoservicio sobre OPS), y recibe avisos automáticos por mail. Las opciones a priorizar de acá en más (orden sugerido, no comprometidas):

- **Promoción coordinada del Carril B a OPS (incluye 9B/3b) — próximo hito del carril:** operación única por DDL — objetos 9C→9G + la **capa con estado de 9H** (5 tablas + 10 triggers + 9 funciones) + `abortar_si_falla(jsonb)` + workflow de cobranza — con bump único del canónico, marcador `'ambiente'='ops'`, GRANTs/RLS a decidir, verificación de socios reales y **sin datos de TEST** (los fixtures no viajan).
...
- Contabilidad productiva en OPS y capa fiscal/legal completa. *El **Carril B** está **completo en TEST (9C-9H)**: catálogo, activación, matriz, gasto rediseñado, **cascada de liquidación validada al centavo (9G)** y **cuenta corriente interna de socios con saldos acumulados, retiros y conversión ARS→USD (9H)**; queda que los socios fijen el % operativo real (decisión de negocio) y la promoción coordinada a OPS. La cobranza posterior 9B/3b (Carril A) es el primer ladrillo que escribe pagos, validado en TEST. **Nada de esto está en OPS ni en el canónico todavía.*** La facturación fiscal (AFIP/ARCA/IVA) queda fuera del Carril B.
