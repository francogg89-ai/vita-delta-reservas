...
- **Bootstrap kit rezagado (P-CC-4).** El kit vigente es `bootstrap_entorno_nuevo_v1.9.0/`, **rezagado respecto del canónico v1.12.0** (deuda acumulada: v1.10.0 lecturas CC + v1.10.1 `pct_operativo` + v1.11.0 SB0/SB1 del retiro + **v1.12.0 snapshot con detalle fino congelado + L3** —3 tablas + 6 triggers + `registrar_snapshot_periodo` extendida + 2 funciones L3—). Se **regenera en el Bloque C de la canonicalización CC** (a la versión final del canónico), no antes.
