...
**Al 2026-06-29:** el sistema opera sobre **OPS** (operación real interna) con **dos carriles promovidos**: el **Carril B** (contabilidad operativa interna entre socios) y el **Carril C — Portal Operativo Interno** (hub de operación con login único, gateway `portal-api`, 13 wrappers n8n y frontend). El **schema canónico está en v1.9.0** (Carril B = **PARTE C**; Carril C / Portal = **PARTE D** + §25) y el **bootstrap kit vigente es `bootstrap_entorno_nuevo_v1.9.0/`**. OPS opera reservas reales (8B), calendarios visuales (8C), bloqueos (8D), avisos por mail (8C-bis) y ahora el **Portal Operativo Interno** (lecturas + escrituras por rol: reservas, cobranzas multi-porción, gastos internos, saldos a cobrar, históricos e ingresos), con **paridad estructural TEST↔OPS del portal certificada por fingerprint**. Los tres entornos **DEV/TEST/OPS** están alineados sobre el canónico (DEV reconstruido desde cero, cerrado como OPS). **Próxima etapa:** operar el Portal Operativo sobre OPS; en **frentes separados** (a abrir cuando se decida), **Mercado Pago** (pagos autónomos) y la **web pública** de reservas. El recorrido del Carril C (diseño → slices → frontend → promoción) está detallado en la sección «Próximas etapas» y en `Docs/Bitacora/PROMOCION_CARRIL_C_OPS_CIERRE.md`.
...
- `Docs/Implementacion/6B_SCHEMA_SQL.md` — schema canónico SQL vigente (**v1.9.0**: v1.8.0 incorporó el Carril B, v1.8.1 canonizó el hardening del motor en Bloque 23, **v1.9.0 incorpora el Carril C / Portal Operativo Interno como PARTE D** —`portal_usuarios`, `portal_idempotencia`, `portal_cargar_gasto_interno(jsonb)`— + §25; ver `Docs/Bitacora/PROMOCION_CARRIL_C_OPS_CIERRE.md`).
