...
- **L-CC-13** — **Fold ASCII de mensajes heredados del canónico al emitir artefactos SQL ASCII-puros.** Al reemplazar `registrar_snapshot_periodo` por `DROP + CREATE`, el cuerpo se preservó byte-a-byte salvo un **fold ASCII de 3 caracteres** (`período`→`periodo` en 1 comentario y 2 `RAISE`), sin cambio de lógica: los artefactos van ASCII-puro, así que los mensajes con acento heredados del canónico se foldean en el artefacto (al canonizar se decide mantener el fold o restaurar acentos). *(Este L-CC-13 pertenece al cierre posterior del snapshot extendido / frente CC completo, 2026-07; no contradice la nota histórica del cierre del retiro —"no se acuñó L-CC-13 / la candidata de `node --check` es L-C-27"—, que reflejaba el estado de ese momento: el slot 13 quedó libre y ahora lo toma esta lección.)*
- **L-CC-14** — **Validación de la ESCRITURA del re-snapshot con variante rollback-first para no ensuciar TEST.** El poblamiento del detalle fino por la función extendida se validó con un run `BEGIN` → gate → congelar con la función real → asserts → `ROLLBACK`: 11/11 PASS sin dejar mutación (0 fotos nuevas, vigente sin cambio, 0 detalle persistido). Un `nextval` bajo `ROLLBACK` deja un hueco de secuencia benigno. *(Mismo patrón, distinto objetivo que L-CC-19: L-CC-14 valida una escritura —el re-snapshot—.)*
- **L-CC-15** — **Verificación ACL real por `aclexplode` cubriendo PUBLIC + los 3 roles Data API y todos los privilegios.** El `04_VERIFY_ESTRUCTURAL` no se conforma con "existe un REVOKE": expande el `proacl`/`relacl` efectivo con `aclexplode` y verifica **0 grants** para `PUBLIC`/`anon`/`authenticated`/`service_role` por cada privilegio, más `proacl_materializado=true`. Ata la ausencia de fuga al ACL real, no a la intención del DDL.
- **L-CC-16** — **Una sola construcción cubre "foto con detalle" y "foto pre-extensión".** Las secciones de detalle fino caen a `[]` por sí solas cuando las tablas están vacías para ese `id_liquidacion`: sin ramas A/B ni código duplicado. Validado en harness con una foto de cada clase corriendo el mismo código.
- **L-CC-17** — **`SUM(SUM(valor_relativo)) OVER ()` da el pool total en una sola pasada tras el `GROUP BY`.** Para el denominador de participación en `matriz_por_socio`, la window sobre el agregado computa el pool total sin segunda consulta ni subquery.
- **L-CC-18** — **Validación de contrato JSON asertiva; `search_path` comparado por valor exacto normalizado.** El verify de las funciones L3 usa `?&` para exigir todas las claves top-level, `jsonb_typeof(...)='array'` para los tipos de sección y sub-checks booleanos por foto — más fuerte y diagnosticable que contar filas. El `search_path` se compara por **valor exacto normalizado** (no basta con que exista un `SET`).
- **L-CC-19** — **Rollback-first que escribe SOLO para fabricar una foto efímera y validar la LECTURA L3 con detalle completo, sin ensuciar TEST.** L3 es **read-only**; el problema es que ni TEST (solo fotos pre-extensión) ni OPS (greenfield) tienen una foto vigente con detalle persistido para ejercitar el caso A. Para probar esa lectura se fabrica una foto efímera con la función real dentro de una txn (`BEGIN` → gate → `CREATE TEMP … ON COMMIT DROP` → congelar → asserts vía `DO` → evidencia → `ROLLBACK`), se corren las lecturas L3 sobre ella (incl. supersesión V2 supersede V1 y acumulados) y se descarta todo con `ROLLBACK`. La escritura es solo un **andamio de fixture**; nada persiste (salvo el hueco de secuencia benigno). *(Mismo patrón que L-CC-14, pero L-CC-14 valida una escritura —el re-snapshot— y L-CC-19 valida una lectura —L3— que ningún entorno tiene con datos reales.)*
