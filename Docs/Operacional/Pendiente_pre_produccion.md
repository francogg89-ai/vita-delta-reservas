...
**Estado del archivo:** actualizado al cierre del **frente Cuenta Corriente — escritura/retiro desde saldo vivo (backend + gateway) + canónico v1.11.0** (2026-07-05). El **Carril B** (contabilidad operativa interna) y el **Carril C** (Portal Operativo Interno) están **promovidos a OPS** por DDL, y sobre ellos se sumó la **capa de escritura de la cuenta corriente** (SB0 + SB1 + gateway A29), también en OPS. **Pendientes activos:** el bootstrap kit rezagado respecto de v1.11.0 (**P-CC-4**); la **UI del Portal para "Retirar saldo"** (frente frontend posterior); el snapshot mensual/congelado + **L3** histórico (remanente de **P-CC-2**); la corrida end-to-end del bootstrap kit sobre un Supabase nuevo; el cosmético de comentarios `D-C-61…64` del gateway A10-MP (solo si se edita ese artefacto); y los pendientes históricos/futuros (RLS para frontend público, tarifas/feriados productivos, capa fiscal AFIP/ARCA, entorno PROD público) listados abajo.
...
- **P-CC-2** — **Frente de escritura: retiro (backend/gateway) HECHO; falta congelado + reembolsos.** **Parcialmente resuelto** por el cierre del **retiro desde saldo vivo (backend + gateway)** en OPS (2026-07-05; SB0 + SB1 + A29). **Sigue pendiente:** (a) el **snapshot mensual/congelado de la cascada completa** del mes —no solo saldos por socio, sino ingresos, gastos, pasos de cascada, matriz/incidencias y resultado por socio— respetando "IA propone, humanos aprueban"; es histórico/auditoría, **no** condición para retirar, y habilita **L3** (que depende del congelado). (b) La **UI del retiro** en el Portal Operativo (botón/pantalla "Retirar saldo"), **frente frontend posterior**. (c) **Reembolsos:** aún no implementados — quedan como **pendiente separado dentro del remanente de P-CC-2** (registro/exposición de reembolsos, análogo al retiro).
...
- **P-CC-4 — Regenerar el bootstrap kit (deuda consciente).** El **canónico vigente está en v1.11.0** pero el kit sigue en `bootstrap_entorno_nuevo_v1.9.0/`, rezagado. Deuda acumulada: **v1.10.0** (`cuenta_corriente_viva`, `cuenta_corriente_detalle` + su `REVOKE`), **v1.10.1** (`pct_operativo_vigente()` + su `REVOKE` + seed `pct_operativo`) y **v1.11.0** (SB0/SB1 del retiro: `portal_usuarios.id_socio` + FK/UNIQUE/CHECK, tabla `portal_idempotencia_cc`, funciones `registrar_retiro_desde_saldo_vivo`/`portal_registrar_retiro` + sus `REVOKE`, y el auto-test **D5 extendido a la 2ª FK**). **Timing:** regenerar cuando cierre el frente completo de cuenta corriente (snapshot mensual + L3), a la versión final del canónico — **no dentro de este patch**. **Riesgo aceptado y acotado mientras no se cree un entorno nuevo desde el kit.** Si hubiera que bootstrappear antes del cierre completo, se debe aplicar el kit `v1.9.0` y luego completar manualmente, desde el canónico vigente, las funciones CC de lectura, el helper/seed `pct_operativo` y la **capa de escritura SB0/SB1** faltantes. Al regenerarlo: los 9 archivos desde el canónico final + el verify estricto, carpeta `bootstrap_entorno_nuevo_vX.Y.Z/`, retirando el kit viejo del árbol (queda en git).
