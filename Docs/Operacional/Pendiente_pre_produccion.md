...
**Estado del archivo:** actualizado al cierre de la **promoción del Carril C (Portal Operativo Interno) a OPS + canónico/kit v1.9.0** (2026-06-29). El **Carril B** (contabilidad operativa interna) y el **Carril C** (Portal Operativo Interno) ya están **promovidos a OPS** por DDL, sin copiar datos de TEST. **Pendientes activos:** corrida end-to-end del bootstrap kit v1.9.0 (01→03) sobre un Supabase nuevo; el cosmético de comentarios `D-C-61…64` del gateway A10-MP (solo si se edita ese artefacto); y los pendientes históricos/futuros (RLS para frontend público, tarifas/feriados productivos, capa fiscal AFIP/ARCA, entorno PROD público) listados abajo.
...
- **P-CC-4** — **Regenerar el bootstrap kit — una sola vez, al cierre del frente de contabilidad completo.** El canónico está en v1.10.0 pero el kit sigue pineado a `bootstrap_entorno_nuevo_v1.9.0/` (le faltan las 2 funciones de cuenta corriente). **Timing decidido:** NO regenerarlo en cada bump intermedio, sino **una sola vez cuando cierre la cuenta corriente completa** (L1/L2 + congelado + L3), a la **versión final** del canónico — el canónico va a volver a bumpear probablemente en **L3** (nueva función de lectura del histórico), y regenerar el kit (9 archivos + `03_VERIFY_FINAL_ENTORNO.sql` estricto) es costoso; regenerarlo ahora sería re-hacerlo en L3. **Bajo riesgo** mientras tanto: lo que falta son **lecturas**, ningún entorno nuevo se crea en el medio, y si hiciera falta bootstrappear uno antes, se corre el kit `v1.9.0` y se aplican encima las funciones de cuenta corriente del canónico. **Al regenerarlo:** los 9 archivos desde el canónico final + el verify estricto, carpeta `bootstrap_entorno_nuevo_vX.Y.Z/`, retirando el kit viejo del árbol (queda en git).
