...
- **P-CC-4 — Regenerar el bootstrap kit (deuda consciente).** El **canónico vigente está en v1.12.0** pero el kit sigue en `bootstrap_entorno_nuevo_v1.9.0/`, rezagado. Deuda acumulada: **v1.10.0** (`cuenta_corriente_viva`, `cuenta_corriente_detalle` + su `REVOKE`), **v1.10.1** (`pct_operativo_vigente()` + su `REVOKE` + seed `pct_operativo`), **v1.11.0** (SB0/SB1 del retiro: `portal_usuarios.id_socio` + FK/UNIQUE/CHECK, tabla `portal_idempotencia_cc`, funciones `registrar_retiro_desde_saldo_vivo`/`portal_registrar_retiro` + sus `REVOKE`, y el auto-test **D5 extendido a la 2ª FK**) y **v1.12.0** (snapshot con detalle fino congelado + L3: 3 tablas append-only `liquidacion_participacion`/`liquidacion_gasto`/`liquidacion_incidencia` + 6 triggers + `registrar_snapshot_periodo` extendida, y las 2 funciones L3 `cuenta_corriente_historico`/`cuenta_corriente_historico_acumulados` + sus `REVOKE` + hardening C12/C14). **Timing:** se resuelve en el **Bloque C de la canonicalización CC** (absorbe la parte de bootstrap de P-L3-03), a la versión final del canónico — **no dentro de este patch**; queda **ABIERTO**. **Riesgo aceptado y acotado mientras no se cree un entorno nuevo desde el kit.** Si hubiera que bootstrappear antes, se debe aplicar el kit `v1.9.0` y luego completar manualmente, desde el canónico vigente, las funciones CC de lectura, el helper/seed `pct_operativo`, la **capa de escritura SB0/SB1** y **las 3 tablas de detalle fino + 6 triggers + el snapshot extendido + las 2 funciones L3** faltantes. Al regenerarlo: los 9 archivos desde el canónico final + el verify estricto, carpeta `bootstrap_entorno_nuevo_vX.Y.Z/`, retirando el kit viejo del árbol (queda en git).
