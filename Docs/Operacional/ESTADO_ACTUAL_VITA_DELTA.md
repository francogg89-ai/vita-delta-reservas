...
**Adición aditiva en TEST (Fase 3b, no canónica todavía):** la función de orquestación `public.abortar_si_falla(jsonb)` existe **solo en TEST**, creada para el rollback transaccional de la cobranza posterior (D-9B-19). Es aditiva (no toca tablas, enums ni `registrar_pago()`), por lo que **no modifica el canónico v1.7.3**; su DDL está documentada en `9B_CIERRE.md` §3. Si en el futuro la cobranza posterior se promueve a OPS, deberá crearse allí primero. La decisión de incorporarla (o no) al canónico se evaluará en el trabajo de schema de contabilidad (Carril B).

---

## Próxima etapa — opciones disponibles

Con 6D, 7A, 7B, 7C, 7D, 7E, **8A, 8B, 8C, 8D cerradas, la Etapa 8 (operación real interna) está completa** y **8C-bis** (alerta por reserva próxima) quedó cerrada y activa en OPS. La **Etapa 9 / Carril A** arrancó: 9A (diagnóstico) y 3a-v2 (listado read-only) cerrados, y **9B / Fase 3b (cobranza posterior multi-porción) cerrada y validada en TEST** (escritura transaccional todo-o-nada). El equipo opera el complejo con tres acciones autoservicio sobre OPS (cargar reservas, ver el estado, crear bloqueos). Las siguientes son **opciones disponibles** a priorizar por Franco (orden sugerido, no comprometidas en este documento):

- **Promover 9B / Fase 3b a OPS:** crear primero `public.abortar_si_falla(jsonb)` en OPS (aditiva; si falta, 3b falla), luego importar el workflow `__OPS` (credencial `vita_supabase_ops`, Basic Auth propia, marcador de entorno) y hacer smoke con datos reales. TEST antes que OPS. Ver `Pendiente_pre_produccion.md`, sección de promoción 9B.
- **Etapa 9 / Carril B — políticas de liquidación:** split operativo 75/25, reparto entre socios, y la liquidación del `extra` (recargo 5%): definir si es repartible, gasto financiero, comisión interna o ingreso separado. Sigue abierto y sin iniciar.
- **Arquitectura global de contabilidad (trabajo de schema futuro):** gastos, caja por lugar, conversión / tabla de ahorro de monedas, cancelaciones con cargo, AFIP/ARCA/IVA/facturación. Implicará una nueva versión del canónico (`6B_SCHEMA_SQL.md`) y su propia ronda de actualización de satélites. Conversación aparte.
