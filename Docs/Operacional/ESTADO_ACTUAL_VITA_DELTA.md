...
- `6B_SCHEMA_SQL.md v1.8.1` — schema canónico SQL vigente (incorpora el Carril B como capa real). Backups históricos archivados.
...
- `Docs/Implementacion/Bootstrap_Entorno_Nuevo_v1.8.1/` — juego repetible para levantar un entorno de cero desde el canónico v1.8.1 (precheck → bootstrap Parte B → verify → bootstrap Parte C → verify final + `README_EJECUCION_BOOTSTRAP.md`); extracción literal del canónico, fijada a v1.8.1 (regenerar en cada bump), validada de punta a punta contra un PostgreSQL limpio (aún no ejecutada sobre un Supabase real). Formaliza la reconstrucción de DEV como herramienta de réplica/recuperación de entornos; solo sobre base vacía.
