...
**Adición aditiva en TEST (Fase 3b, no canónica todavía):** la función de orquestación `public.abortar_si_falla(jsonb)` existe **solo en TEST**, creada para el rollback transaccional de la cobranza posterior (D-9B-19). Es aditiva (no toca tablas, enums ni `registrar_pago()`), por lo que **no modifica el canónico v1.7.3**; su DDL está documentada en `9B_CIERRE.md` §3. Si en el futuro la cobranza posterior se promueve a OPS, deberá crearse allí primero. La decisión de incorporarla (o no) al canónico se evaluará en la promoción coordinada del Carril B.

**Adiciones aditivas en TEST (Carril B 9C→9G, no canónicas todavía):** columnas `cabanas.valor_relativo` y `cabanas.id_socio_beneficiario`; tablas `zonas`, `cabana_zona`, `activaciones_operativas` (con su EXCLUDE gist) y `gastos_internos`; la clave `configuracion_general('ambiente')`; y diez funciones read-only (`resolver_beneficiario`, las 3 de 9E y las 6 de 9G), todas `STABLE SECURITY INVOKER` con REVOKE de los cuatro roles Data API. **Nada de esto modifica el canónico v1.7.3 ni existe en DEV/OPS.** La incorporación al canónico será un **bump único** en la promoción coordinada del Carril B, que también resolverá el destino de la `gastos` legacy (D-9F-01) y los GRANTs operativos. Los fixtures de laboratorio (`seed_9f_validacion`, pagos `seed_9g_%`) no son schema y **no viajan**.

---

## Próxima etapa — opciones disponibles

Con 6D, 7A, 7B, 7C, 7D, 7E, **8A, 8B, 8C, 8D cerradas, la Etapa 8 (operación real interna) está completa** y **8C-bis** (alerta por reserva próxima) quedó cerrada y activa en OPS. La **Etapa 9 / Carril A** arrancó: 9A (diagnóstico) y 3a-v2 (listado read-only) cerrados, y **9B / Fase 3b (cobranza posterior multi-porción) cerrada y validada en TEST** (escritura transaccional todo-o-nada). El **Carril B cerró su capa derivada (9C→9G) en TEST**: cascada de liquidación read-only validada al centavo (40/40), sin nada promovido. El equipo opera el complejo con tres acciones autoservicio sobre OPS (cargar reservas, ver el estado, crear bloqueos). Las siguientes son **opciones disponibles** a priorizar por Franco (orden sugerido, no comprometidas en este documento):

- **Etapa 9H — Carril B, capa acumulada:** saldos acumulados entre períodos, retiros de socios, revaluación y compensación pagador↔incidido (las deudas Rodrigo↔Franco del termotanque y Remo↔Rodrigo de las horas ya quedaron a la vista en 9G). Insumos listos: las 6 funciones de cascada, el banco de laboratorio (fixtures 9F+9G) y `PENDIENTE_CARRIL_B_9H_SALDOS_RETIROS_REVALUACION.md`. Conversación nueva.
- **Promoción coordinada del Carril B a OPS (incluye 9B/3b):** paquete único por DDL — objetos 9C/9D/9E/9F/9G + `abortar_si_falla(jsonb)` + workflow 3b `__OPS` — con bump del canónico, marcador `'ambiente'='ops'` (D-9C-19), GRANTs/RLS a decidir, verificación de socios reales (L-9C-01) y **sin datos de TEST**. Ver `Pendiente_pre_produccion.md`.
- **Decisiones de negocio del Carril B (socios, fuera del sistema):** el valor real del % operativo (las funciones lo reciben por parámetro; 0,25 fue valor de trabajo) y la política de arranque para lo percibido con pool vacío (junio 2026: base de ganancia derivada de $1.023.900 sin destinatarios). La liquidación del `extra` (5%) quedó **resuelta por diseño**: ingreso post-operativo, paso 6 de la cascada (conceptual §4.3 / D-9G-02), comparado contra el monotributo sin netear (reporte 5-vs-fiscal).
- **Resto de la arquitectura global de contabilidad:** caja por lugar, conversión / tabla de ahorro de monedas, cancelaciones con cargo, AFIP/ARCA/IVA/facturación (carril fiscal, separado por diseño). El bump del canónico (`6B_SCHEMA_SQL.md`) llega con la promoción coordinada del Carril B. Conversación aparte.
