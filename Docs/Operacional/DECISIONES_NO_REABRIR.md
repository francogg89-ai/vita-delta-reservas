...
## Frente Cuenta Corriente de socios — snapshot con detalle fino congelado + L3 histórico — cerrada 2026-07-07

Cierre estructural de la capa de **congelado con detalle fino + lectura histórica** de la cuenta corriente: snapshot mensual extendido (Bloque 1) y lecturas L3 read-only (Bloque 2). Mismo namespace `D-CC-XX`; canónico v1.12.0; promovido a OPS **solo estructura** (greenfield en datos). Cierres: `EXT_SNAPSHOT_BLOQUE1_CIERRE.md`, `L3_HISTORICO_BLOQUE2_CIERRE.md`.

- **D-CC-23** — **Opción B: el snapshot congela también el detalle fino, en la misma txn/lock que cascada+socios.** `registrar_snapshot_periodo` se extiende (aditivo) para que, además del esqueleto 9H (cabecera + cascada 1–8 + saldos por socio), congele el detalle fino de P-CC-2: participación por cabaña, gastos individuales e incidencias por gasto. El bloque de extensión corre después de los INSERT de cascada y socios, dentro de la **misma transacción** y el mismo `pg_advisory_xact_lock`. Se prefirió el snapshot completo a un L3 recortado porque OPS estaba greenfield (sin fotos incompletas que migrar).
- **D-CC-24** — **Tres tablas append-only con PKs compuestas (sin secuencias); `liquidacion_gasto` = foto fiel de `gastos_internos`; matriz y nombres derivados.** `liquidacion_participacion` (grano cabaña), `liquidacion_gasto` (grano gasto, las 19 columnas de la fila + `sin_incidencia`/`motivo_sin_incidencia` congelados) y `liquidacion_incidencia` (grano incidencia, con `regla`). PKs compuestas → **0 secuencias nuevas**. La matriz por socio y los nombres de cabaña/socio **no se persisten**: se derivan por agregación/join.
- **D-CC-25** — **Inmutabilidad, REVOKE y supersesión idénticas a la capa 9H.** Dos triggers por tabla (BEFORE UPDATE/DELETE por fila + BEFORE TRUNCATE por statement, reusando `trg_9h_inmutable()`), `REVOKE ALL` frente a `PUBLIC/anon/authenticated/service_role` (0 Data API) y la misma cadena de supersesión. El ARRAY de C6 pasa de 5 a **8 tablas (16 triggers)**.
- **D-CC-26** — **Re-snapshot: cada versión de foto posee su set completo de detalle; la superseded queda congelada y fuera de las lecturas vigentes; nada se borra ni actualiza.** Al re-congelar, la nueva foto persiste su propio detalle completo; el de la superseded permanece congelado pero deja de leerse (L3 resuelve por `liquidacion_vigente`). Append-only puro.
- **D-CC-27** — **Detalle congelado pct-independiente; solo la cascada usa `pct_operativo_vigente()`.** Participación, gastos e incidencias congelados no dependen del `pct`; el porcentaje solo interviene en la cascada (paso 4). Cambiar el `pct` a futuro no altera un detalle ya congelado.
- **D-CC-28** — **Fotos anteriores a la extensión quedan sin detalle; L3 tolera detalle vacío; OPS nace con detalle completo.** Las pre-extensión (solo fixtures TEST) no tienen filas de detalle → L3 devuelve `[]`. OPS, greenfield, nace con la primera foto real ya con detalle.
- **D-CC-29** — **`regla` congelada en T3; pagador congelado en T2; `motivo_sin_incidencia` congelado en T2; nombres derivados.** `regla` en `liquidacion_incidencia`; `pagador_tipo`/`id_socio_pagador` y `motivo_sin_incidencia` en `liquidacion_gasto`. Los nombres legibles se derivan por join en la lectura.
- **D-CC-30** — **`liquidacion_gasto.id_gasto` = referencia histórica copiada sin FK a `gastos_internos`; FK `RESTRICT` solo a catálogos estables; FK compuesta T3→T2 entre tablas congeladas.** El `id_gasto` se copia como referencia histórica **sin FK** a `gastos_internos` (fuente mutable → foto autocontenida: borrar el gasto vivo no toca la foto). Las FK `ON DELETE RESTRICT` van solo a catálogos estables (`socios`/`cabanas`/`zonas`); `liquidacion_incidencia (id_liquidacion, id_gasto)` → `liquidacion_gasto` (FK compuesta entre tablas ya congeladas).
- **D-CC-31** — **L3 es read-only sobre la foto vigente: resuelve vía `liquidacion_vigente(mes)`, nunca lee superseded; sin escrituras ni efectos.** Las 2 funciones (`cuenta_corriente_historico(date)` / `cuenta_corriente_historico_acumulados()`) solo leen; no tocan L1/L2 ni los retiros ni la foto.
- **D-CC-32** — **Dos ramas top-level (`sin_foto` / foto-existe); `detalle_disponible`/`detalle_motivo` se derivan de la cardinalidad de `liquidacion_participacion`; no se ramifica por A/B.** Casos A (con detalle) y B (pre-extensión) comparten construcción: las secciones de detalle salen `[]` solas cuando las tablas están vacías (0 participación ⇔ pre-extensión).
- **D-CC-33** — **Convención de signos fiel a la foto congelada; L3 no transforma.** Ingresos +, gastos − (deducciones), utilidad neta con su signo, retiros con signo. `gastos_acumulados` es la suma de deducciones (negativa). L3 no re-signa ni normaliza.
- **D-CC-34** — **`movimientos` en L3-detalle es lectura VIVA del mayor, ventaneada por `fecha ∈ [mes, mes+1)`; no forma parte del congelado.** Es la única sección no congelada del detalle; las otras 8 leen la foto.
- **D-CC-35** — **`matriz_por_socio` y `gastos_sin_incidencia` son derivados; no reimplementan las funciones vivas.** La matriz se deriva de `liquidacion_participacion`; "gastos sin incidencia" del filtro `sin_incidencia` sobre `liquidacion_gasto`. L3 no invoca las funciones vivas de matriz/incidencias.
- **D-CC-36** — **L3 fija `search_path = pg_catalog, public` (a diferencia de L1/L2), es `SECURITY INVOKER`, `STABLE`, y `REVOKE EXECUTE` a los 3 roles Data API + `PUBLIC`.**
- **D-CC-37** — **El piso no se doble-filtra en L3-acumulados: `saldo_corriente_socio` se reutiliza verbatim; el piso 2026-07-01 solo se expone como check en `meta`.** Suma todas las fotos vigentes y todos los movimientos sin recorte; el piso (reafirma D-NEG-02) aparece solo como `fotos_pre_piso`/`movimientos_pre_piso` (deben ser 0). Una sola fuente de verdad del saldo vivo.
- **D-CC-38** — **`saldos_por_socio` reutiliza `saldo_corriente_socio` verbatim (vía LATERAL, pivot de las 4 componentes).** En vez de reimplementar el saldo vivo, llama la función por socio y pivotea `resultado_liquidacion`/`reembolso_desembolso`/`movimientos`/`saldo_vivo`.
- **D-CC-39** — **`retribucion_operativo` presente cuando hay foto (A y B), `null` en `sin_foto` (C).** La conciliación del paso 4 (vía `reporte_retribucion_operativo_periodo`) se incluye siempre que exista foto vigente.
