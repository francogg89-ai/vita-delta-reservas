# ============================================================================
# C_SLICE1_B8_smoke_a06_directo.ps1
# Carril C / Portal Operativo Interno - Slice 1, Bloque 8 (wrapper de A06).
# Smoke DIRECTO al wrapper n8n "portal-a06-prereservas" (TEST). Defensa en profundidad:
...
# n8n recomputa el HMAC sobre el raw body recibido (D-C-29). A06 NO lleva payload (payloadVacio).
#
...
#
# IMPORTANTE: el happy path (vicky/socio) PASA aunque filas.length = 0. Cero prereservas
# activas es resultado VALIDO (D-C-47): ok:true con data.filas = []. NO se crean fixtures.
# (Para saber cuantas activas hay en TEST, ver la query read-only del runsheet B8.)
#
...
$Webhook = "portal-a06-prereservas"                   # path del Webhook del wrapper
$Secret  = "SECRETO NO COMMMITEAR"          # == secreto del nodo; NO commitear
# =============================
...
function New-Body {
  ...
  # JSON armado a mano para controlar los bytes EXACTOS que se firman y se envian. payload={} (A06).
...
$ACT = "prereservas.activas"        # EXPECTED_ACTION del wrapper
...
# --- 3: jenny (rol valido del portal, NO habilitado para A06)  ->  rol_no_permitido ---
...
$b = New-Body -Action "calendario.operativo" -Rol "vicky" -AmbienteEsperado "test" -Ts $now -Nonce ([guid]::NewGuid().ToString())
...
Write-Host "`nListo. 8 casos. Compara cada veredicto con su 'esperado' (criterios en el runsheet B8)." -ForegroundColor Green
