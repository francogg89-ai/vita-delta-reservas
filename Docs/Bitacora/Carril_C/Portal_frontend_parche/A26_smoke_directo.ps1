...
# Smoke DIRECTO contra el wrapper n8n portal-a26-disponibilidad (sin gateway).
# 100% TEST. READ-ONLY: A26 es lectura, no consume secuencias (D-PROMO-09).
#
...
#
# USO:
#   1) Pega el secreto HMAC real de TEST en $Secret.
#   2) Ajusta $CabValida / $CabInvalida si en TEST no son 1 / 999999.
#   3) (opcional) Completa $OcupDesde/$OcupHasta/$FechaCheckout desde
#      A26_smoke_oracle.sql para activar T5/T6 (si quedan vacios, se SALTEAN).
...
$Secret    = 'd3cb37c88b688c6e104f133d08990312134ee5df6775e2dff267e0deac16c3f4'        # mismo secreto del wrapper (Modo B)
$BaseUrl   = 'https://federicosecchi.app.n8n.cloud'
$Webhook   = 'portal-a26-disponibilidad'
$WebhookSeg= 'webhook'                             # 'webhook' (activo) o 'webhook-test' (editor)
$Ambiente  = 'test'                               # ambiente_esperado del sobre (== configuracion_general)

$CabValida   = 1                                  # id de una cabana ACTIVA en TEST
...
$CabOcup       = 5
$OcupDesde     = '2026-07-08'                               # 'yyyy-MM-dd' inclusive
$OcupHasta     = '2026-07-16'                               # 'yyyy-MM-dd' exclusive
$FechaCheckout = '2026-07-15'                               # 'yyyy-MM-dd' marcado checkout_disponible
...
if ($Secret.StartsWith('__PEGAR_')) {
  Write-Host 'ERROR: pega el secreto HMAC real de TEST en $Secret.' -ForegroundColor Red
  exit 2
}
...
# ===== Cabecera ===========================================================
Write-Host ''
Write-Host '=== A26 smoke directo (TEST, read-only) ===' -ForegroundColor Cyan
...
Test-Case 'T10 ambiente_esperado=ops -> ambiente_incorrecto' {
  $r = Invoke-A26 -Payload @{ id_cabana=$CabValida; fecha_desde=$LibreDesde; fecha_hasta=$LibreHasta } -AmbEsperado 'ops'
