...
--
-- MARCADOR: el smoke usa idempotency_key con prefijo 'smoke-a11gw-'. La traza
--   vive en portal_idempotencia (idempotency_key LIKE 'smoke-a11gw-%') y apunta
...
BEGIN
  ...
  FROM public.portal_idempotencia WHERE idempotency_key LIKE 'smoke-a11gw-%';
  SELECT count(*) INTO v_pre_gastos
  FROM public.gastos_internos g
  WHERE g.id_gasto IN (
    SELECT pi.id_gasto FROM public.portal_idempotencia pi
    WHERE pi.idempotency_key LIKE 'smoke-a11gw-%' AND pi.id_gasto IS NOT NULL
  );
...
WITH borradas AS (
  ...
  WHERE idempotency_key LIKE 'smoke-a11gw-%'
  RETURNING id_gasto
)
...
SELECT
  jsonb_build_object(
    'trazas_smoke_restantes',
      (SELECT count(*) FROM public.portal_idempotencia WHERE idempotency_key LIKE 'smoke-a11gw-%'),
    'fixture_9f_intacto',
      (SELECT count(*) FROM public.gastos_internos WHERE creado_por = 'seed_9f_validacion')
  ) AS detalle,
  CASE
    WHEN (SELECT count(*) FROM public.portal_idempotencia WHERE idempotency_key LIKE 'smoke-a11gw-%') = 0
    THEN 'PASS'
    ELSE 'FAIL'
  END AS veredicto;
