...
### 2.D — No-regresión A07/A08 (camino normal con fechas futuras)

**Modelo de noche intacto:** confirmado por 2.A.4 (EXCLUDE idéntico). El guard solo rechaza `fecha_in`/rango pasados; **no** toca la adyacencia checkout=checkin.

**Probes SIN consumo (fechas futuras pasan el guard).** Estas dos llamadas usan fechas futuras y se rechazan por una validación *posterior* al guard, que dispara **antes** de cualquier `INSERT`/`nextval`. Prueban que el guard deja pasar lo futuro hacia la lógica normal (no alterada, por el diff). Cero secuencia, cero datos. `BEGIN/ROLLBACK` como red de seguridad.

```sql
-- 2.D.1 crear_prereserva con fecha futura → NO debe devolver fecha_in_pasada.
-- huesped sin telefono ni email a propósito → corta en huesped_contacto_requerido (línea 3727,
-- ANTES de upsert_huesped en 3779). Si saliera 'fecha_in_pasada', el guard estaría rompiendo lo futuro.
BEGIN;
SELECT crear_prereserva(jsonb_build_object(
  'id_cabana', 1,
  'fecha_in',  (fecha_hoy_ar() + 10)::text,
  'fecha_out', (fecha_hoy_ar() + 12)::text,
  'personas', 2, 'monto_total', 1000, 'monto_sena', 500,
  'canal_origen', 'test_b2d', 'canal_pago_esperado', 'efectivo',
  'source_event', 'b2d_noregresion',
  'huesped', jsonb_build_object('nombre','Smoke 2D')
)) AS resultado;
ROLLBACK;
-- Esperado: resultado->>'error' = 'huesped_contacto_requerido'  (NO 'fecha_in_pasada').

-- 2.D.2 crear_bloqueo con rango futuro → NO debe devolver rango_pasado.
-- motivo fuera del enum a propósito → corta en motivo_invalido (línea 4428, primer check tras el
-- guard, antes de cabaña/solapamientos/INSERT). Si saliera 'rango_pasado', el guard rompería lo futuro.
BEGIN;
SELECT crear_bloqueo(jsonb_build_object(
  'id_cabana', NULL,
  'fecha_desde', (fecha_hoy_ar() + 10)::text,
  'fecha_hasta', (fecha_hoy_ar() + 12)::text,
  'motivo', '_invalido_2d_',
  'creado_por', 'smoke_b2d',
  'source_event', 'b2d_noregresion'
)) AS resultado;
ROLLBACK;
-- Esperado: resultado->>'error' = 'motivo_invalido'  (NO 'rango_pasado').
```

**Por qué esto cierra 2.D sin end-to-end:** (1) el diff prueba que el guard es puramente aditivo — el camino de aceptación es byte-idéntico al previo; (2) 2.D.1/2.D.2 observan que lo futuro **pasa** el guard hacia la validación siguiente; (3) 2.A.4 confirma el EXCLUDE intacto; (4) 2.B ya probó que las funciones ejecutan. Para una fecha futura, el guard se saltea (observado) y corre el camino de aceptación sin cambios (diff) sobre el EXCLUDE intacto (2.A.4) → no hay regresión.

**Gold-standard opcional (NO requerido por 2.D, destructivo):** un `crear_prereserva`/`crear_bloqueo` con datos válidos reales que devuelva `ok:true` crearía filas y consumiría `id_pre_reserva`/`id_huesped`/`id_bloqueo` de forma **irreversible** (secuencias no transaccionales), y requiere una cabaña activa real + fechas libres. Si lo querés, se pide aparte con payload exacto + cleanup completo.
...
7. **2.D** — probes sin consumo: `crear_prereserva` futura → `huesped_contacto_requerido` (no `fecha_in_pasada`); `crear_bloqueo` futuro → `motivo_invalido` (no `rango_pasado`).
