...
function textoErrorReserva(error: PortalApiError): string {
  if (error.code === 'conflicto') {
    if (error.message.startsWith('gap_checkin:')) return error.message.slice('gap_checkin:'.length).trim();
    if (error.message.startsWith('gap_checkout:')) return error.message.slice('gap_checkout:'.length).trim();
