...
BEGIN
  ...
  v_ocurr := (length(v_def) - length(replace(v_def, v_bloque, ''))) / NULLIF(length(v_bloque), 0);
  IF v_ocurr <> 1 THEN
    RAISE EXCEPTION 'D-RB: el bloque [B1.3-D] aparece % vez/veces (esperado 1). Abortando.', v_ocurr;
  END IF;

  -- Quitar el bloque insertado. El texto original se restaura exactamente: el salto de linea
  -- previo y la linea del INSERT (con su indentacion real) quedan intactos.
  v_base := replace(v_def, v_bloque, '');
