...
# Smoke DIRECTO al wrapper n8n "portal-a28-cuenta-corriente-detalle" (TEST), SIN gateway.
#
...
$Webhook = "portal-a28-cuenta-corriente-detalle"
$Secret  = "d3cb37c88b688c6e104f133d08990312134ee5df6775e2dff267e0deac16c3f4"
$MesOk   = "2026-07-01"   # mes con actividad en TEST
# =============================
...
Assert-OkData "1. socio OK (ok:true, data presente)" (Sig-Send -Payload @{mes=$MesOk} -Rol "socio" -Amb "test" -Ts $now) { param($d) $null -ne $d }
Assert-Code "2. vicky -> rol_no_permitido (socio-only)" (Sig-Send -Payload @{mes=$MesOk} -Rol "vicky" -Amb "test" -Ts $now) "rol_no_permitido"
Assert-Code "3. jenny -> rol_no_permitido" (Sig-Send -Payload @{mes=$MesOk} -Rol "jenny" -Amb "test" -Ts $now) "rol_no_permitido"
Assert-Code "4. firma equivocada -> firma_invalida" (Sig-Send -Payload @{mes=$MesOk} -Rol "socio" -Amb "test" -Ts $now -Key "SECRETO_MALO") "firma_invalida"
Assert-Code "5. ts viejo -> ts_fuera_de_ventana" (Sig-Send -Payload @{mes=$MesOk} -Rol "socio" -Amb "test" -Ts ($now - 600000)) "ts_fuera_de_ventana"
Assert-Code "6. ambiente cruzado -> ambiente_incorrecto" (Sig-Send -Payload @{mes=$MesOk} -Rol "socio" -Amb "ops" -Ts $now) "ambiente_incorrecto"
Assert-Code "7. accion equivocada -> accion_desconocida" (Sig-Send -Payload @{mes=$MesOk} -Rol "socio" -Amb "test" -Ts $now -Act "cobranza.saldos") "accion_desconocida"

# ============================ PAYLOAD {mes} ============================
Write-Host "`n----- PAYLOAD (mes) -----" -ForegroundColor Magenta
Assert-Code "8. sin mes -> payload_invalido" (Sig-Send -Payload @{} -Rol "socio" -Amb "test" -Ts $now) "payload_invalido"
Assert-Code "9. mes mal formado -> payload_invalido" (Sig-Send -Payload @{mes="2026-13-99"} -Rol "socio" -Amb "test" -Ts $now) "payload_invalido"
Assert-Code "10. mes pre-piso (2026-05-01) -> payload_invalido" (Sig-Send -Payload @{mes="2026-05-01"} -Rol "socio" -Amb "test" -Ts $now) "payload_invalido"
Assert-Code "11. clave no permitida -> payload_invalido" (Sig-Send -Payload @{mes=$MesOk; foo=1} -Rol "socio" -Amb "test" -Ts $now) "payload_invalido"

# ============================ FUNCIONAL ============================
Write-Host "`n----- FUNCIONAL -----" -ForegroundColor Magenta
$rF = Sig-Send -Payload @{mes=$MesOk} -Rol "socio" -Amb "test" -Ts $now
