...
export const F9 = fotoE1({
  ...
  // DOS gastos, no uno: un gasto que incidio NO puede figurar tambien en `gastos_sin_incidencia`.
  // La version anterior ponia el #42 en las dos tablas para "probar la correlacion", y con eso el
  // fixture se contradecia con el contrato. La correlacion se prueba igual -- mejor, incluso --
  // con un gasto que incidio (#42) y otro que no (#43).
  gastos: [
    gasto({ id_gasto: 42 }),
    gasto({
      id_gasto: 43,
      etiqueta: 'Poda de la zona norte',
      sin_incidencia: true,
      motivo_sin_incidencia: 'zona_sin_activas',
    }),
  ],
  incidencias: [
  ...
  gastos_sin_incidencia: [
    { id_gasto: 43, clase: 'D', etiqueta: 'Poda de la zona norte', monto: 45000, motivo: 'zona_sin_activas' },
  ],
});
...
export const F20 = fotoE1({
  gastos: [
    gasto({
      ...
      clase: 'E', // clase E = por CABAÑA -> id_cabana no-null, id_zona null
      id_zona: null,
      id_cabana: 5,
      clase_sugerida: 'A',
      comentario:
    ...
    gasto({
      ...
      clase: 'D', // clase D = por ZONA -> id_zona no-null, id_cabana null
      id_zona: 3,
      id_cabana: null,
      clase_sugerida: 'C',
    ...
    // #73 figura en `gastos_sin_incidencia`, asi que el gasto TIENE que declararlo. Antes heredaba
    // el default `sin_incidencia:false` y quedaba contradiciendo a la propia tabla de abajo.
    gasto({
      id_gasto: 73,
      etiqueta: 'Compra menor de ferretería',
      comentario: null,
      comprobante_url: null,
      sin_incidencia: true,
      motivo_sin_incidencia: 'pool_vacio',
    }),
  ],
  incidencias: [
    // La regla tiene que seguir a la CLASE del gasto, no al reves:
    //   clase E (por cabaña) -> 'cabana_directa'
    //   clase D (por zona)   -> 'zona_prorrateo_valor_relativo'
    { id_gasto: 71, seq: 1, destino: 'socio', id_socio: 2, socio: 'Rodrigo', monto_incidido: 45000, regla: 'cabana_directa' },
    { id_gasto: 72, seq: 1, destino: 'socio', id_socio: 3, socio: 'Remo', monto_incidido: 45000, regla: 'zona_prorrateo_valor_relativo' },
  ],
  gastos_sin_incidencia: [
