...
# -*- coding: utf-8 -*-
"""
SB-UI-4-FIX -- Contenido de la Foto del mes (A30). Corrige los 4 puntos de la auditoria:

  P1  comprobante_url NUNCA se pasa crudo a un href (era un XSS almacenado) -> comprobanteSeguro().
  P2  ID congelado + nombre VIVO en socios / participacion / matriz / incidencias, con la nota que
      aclara que los nombres se resuelven contra el catalogo ACTUAL (joins vivos en A30).
  P3  id_gasto como primera columna de Gastos congelados (correlaciona con las otras 2 tablas).
  P4  procedencia congelada del gasto (creado_por + created_at).

Los DOS archivos nuevos CAMBIARON de contenido respecto de SB-UI-4: los hashes de G2 estan
actualizados. HistoricoVista.tsx NO cambia (su edit es identico al de SB-UI-4): si ya esta en POST,
el patcher lo detecta y no lo toca.
...
SHA_NUEVOS = {
    ...
    'screens/historico/ContenidoFoto.tsx':
        'b7658b9eb8ddf7bd4ceaff5500d40b0a5c766aae281669b219c4c5321843eae5',
}
