-- ============================================================================
-- EXT_SNAPSHOT_05_VALIDACION_FUNCIONAL_TEST_ROLLBACK.sql   (Run 5/6 -- ROLLBACK)
-- Valida el detalle congelado SIN dejar mutacion: re-snapshotea un mes dentro de
-- una transaccion, corre 11 checks, y hace ROLLBACK (no queda foto nueva en TEST).
-- TEST-only (gate). NO borra gastos. Mes objetivo: cambiar DATE '2026-07-01' si hace falta.
-- Correr con NADA seleccionado. El ultimo SELECT (antes del ROLLBACK) es el veredicto.
-- Si despues se quiere persistir evidencia, se emite un 05B_COMMIT explicito.
-- ============================================================================
BEGIN;
DO $gate$ BEGIN
...
-- temp table dentro de la txn (se descarta en el ROLLBACK junto con el re-snapshot)
CREATE TEMP TABLE _ext_val (mes date, id_previo bigint, id_nuevo bigint);
...
UPDATE _ext_val SET id_nuevo = registrar_snapshot_periodo(
         mes, pct_operativo_vigente(), 'validacion_ext', id_previo, 'validacion extension detalle fino (ROLLBACK)')
 WHERE mes = DATE '2026-07-01';

-- ---------------- VEREDICTO (dentro de la txn, antes del ROLLBACK) ----------------
SELECT * FROM (
  ...
  SELECT 5, 'sin_incidencia(T2) == gastos_sin_incidencia_periodo',
    CASE WHEN (SELECT count(*) FROM (
         SELECT id_gasto FROM liquidacion_gasto WHERE id_liquidacion=(SELECT id_nuevo FROM _ext_val) AND sin_incidencia
     ...
     AND (SELECT count(*) FROM (
         ...
         EXCEPT SELECT id_gasto FROM liquidacion_gasto WHERE id_liquidacion=(SELECT id_nuevo FROM _ext_val) AND sin_incidencia) b) = 0
    THEN 'PASS' ELSE 'FAIL' END
  UNION ALL
  ...
  SELECT 7, 'sin_incidencia(T2)==true  <=>  0 filas en T3',
    CASE WHEN (SELECT count(*) FROM (
         SELECT id_gasto FROM liquidacion_gasto WHERE id_liquidacion=(SELECT id_nuevo FROM _ext_val) AND sin_incidencia
         EXCEPT
         SELECT g.id_gasto FROM liquidacion_gasto g WHERE g.id_liquidacion=(SELECT id_nuevo FROM _ext_val)
     ...
     AND (SELECT count(*) FROM (
         ...
         EXCEPT
         SELECT id_gasto FROM liquidacion_gasto WHERE id_liquidacion=(SELECT id_nuevo FROM _ext_val) AND sin_incidencia) y) = 0
    THEN 'PASS' ELSE 'FAIL' END
  UNION ALL
  SELECT 8, 'T3 == incidencia_gasto por gasto (destino,socio,monto,regla)',
    CASE WHEN NOT EXISTS (
      SELECT 1 FROM (SELECT id_gasto,destino,id_socio,monto_incidido,regla
                     FROM liquidacion_incidencia WHERE id_liquidacion=(SELECT id_nuevo FROM _ext_val)) f
      FULL JOIN (SELECT g.id_gasto, i.destino, i.id_socio, i.monto AS monto_incidido, i.regla
                 FROM gastos_internos g CROSS JOIN LATERAL incidencia_gasto(g.id_gasto) i
                 WHERE g.periodo=(SELECT mes FROM _ext_val)) v
      ON f.id_gasto=v.id_gasto AND f.destino=v.destino AND f.id_socio IS NOT DISTINCT FROM v.id_socio
      WHERE f.monto_incidido IS DISTINCT FROM v.monto_incidido OR f.regla IS DISTINCT FROM v.regla
  ...
  SELECT 9, 'matriz por socio DERIVADA de T1 == matriz_participacion',
    CASE WHEN NOT EXISTS (
      SELECT 1 FROM (
        SELECT id_socio_beneficiario AS id_socio, SUM(valor_relativo) AS valor_socio,
        ...
        FROM liquidacion_participacion WHERE id_liquidacion=(SELECT id_nuevo FROM _ext_val) AND participa
