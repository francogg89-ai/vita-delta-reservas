...
# Smoke DIRECTO al wrapper n8n "portal-a27-cuenta-corriente" (TEST), SIN gateway.
#
...
$b = New-Body -Action $ACT -Payload @{} -Rol "socio" -AmbienteEsperado "test" -Ts ($now - 600000) -Nonce (NG)
$r = Send-Probe -Body $b -Signature (Get-Signature -Body $b -Key $Secret)
Assert-Code "6. ts viejo -> ts_fuera_de_ventana" $r "ts_fuera_de_ventana"

# 7. ambiente cruzado (ops) -> ambiente_incorrecto
$b = New-Body -Action $ACT -Payload @{} -Rol "socio" -AmbienteEsperado "ops" -Ts $now -Nonce (NG)
$r = Send-Probe -Body $b -Signature (Get-Signature -Body $b -Key $Secret)
...
$b = New-Body -Action "cobranza.saldos" -Payload @{} -Rol "socio" -AmbienteEsperado "test" -Ts $now -Nonce (NG)
$r = Send-Probe -Body $b -Signature (Get-Signature -Body $b -Key $Secret)
