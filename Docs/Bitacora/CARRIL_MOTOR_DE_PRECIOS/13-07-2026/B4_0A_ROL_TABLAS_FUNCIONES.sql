-- ============================================================================
-- B4_0A_v5_ROL_TABLAS_FUNCIONES.sql   (v5 -- post-auditoria 5)
...
--   precios_api, NO en public. El rol recibe USAGE en precios_api y NO en public.
...
-- Si hay residuos: correr B4_0A_v5_ROLLBACK.sql primero.
--
...
BEGIN
  ...
  IF v_det <> '' THEN
    RAISE EXCEPTION 'B4-0A abortado: instalacion FRESH-ONLY y hay residuos ->%. Corre B4_0A_v5_ROLLBACK.sql primero.', v_det;
  END IF;
END
$residuos$;
...
-- El rol NO recibe USAGE en public. No lo necesita: sus entrypoints estan en
...
BEGIN
  ...
  BEGIN
    ...
    -- =======================================================================
    -- FAST PATH -- RECOVERY / REPLAY.  ANTES de cobrar cuota.
    --   Si ya hay una admision para este (sujeto, nonce):
    --     mismo binding    -> devolver el MISMO ticket_id. Sin cuota. Sin nonce.
    --     binding distinto -> nonce_replay (conflicto real).
    -- =======================================================================
    SELECT * INTO v_prev
    ...
    -- =======================================================================
    -- SLOW PATH -- primera admision de este nonce.
    --   La cuota se cobra ACA: un recovery nunca llega a este punto.
    --   Si la cuota RECHAZA, no se escribe el ledger -> el nonce queda LIBRE
    --   (un request rate-limited no debe quemar el nonce del cliente).
    -- =======================================================================
    ...
    -- ===== SUB-SAVEPOINT: ledger + ticket, atomicos entre si =====
    --   CARRERA CONCURRENTE: dos admisiones simultaneas con el mismo nonce. El
    --   FAST PATH no ve la fila de la otra (aun no commiteada) -> las dos llegan
    --   aca. El INSERT del ledger BLOQUEA (medido: espera al xmax) y la perdedora
    --   recibe unique_violation -> lee la fila del ganador -> devuelve SU ticket.
    --   Resultado: UN SOLO TICKET. Revertir esto NO devuelve cuota.
    BEGIN
    ...
    EXCEPTION WHEN unique_violation THEN
      ...
      -- perdi la carrera: el ganador ya commiteo su ledger + su ticket.
      SELECT * INTO v_prev
        FROM precios_api.api_precios_admision_leer(p_sujeto, p_nonce);

      IF NOT FOUND THEN
        -- el ganador aborto entre el conflicto y esta lectura: estado no
        -- reconstruible con este nonce. La Edge debe usar uno nuevo.
        RETURN pg_catalog.jsonb_build_object('ok', false, 'error', 'nonce_replay');
      END IF;
      IF v_prev.admission_hash IS DISTINCT FROM v_hash THEN
        RETURN pg_catalog.jsonb_build_object('ok', false, 'error', 'nonce_replay');
...
-- ============================================================================
-- FIN B4-0A.
-- Siguiente: el ALTER de password (runsheet paso 2) y despues B4_0A_v5_VERIFY.sql.
-- ============================================================================
