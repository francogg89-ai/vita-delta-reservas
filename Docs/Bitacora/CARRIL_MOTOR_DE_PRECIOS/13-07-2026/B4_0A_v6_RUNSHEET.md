# B4-0A -- RUNSHEET   (v6 -- post-auditoria 6)
...
|---|---|---|---|
| 1 | `B4_0A_v6_ROL_TABLAS_FUNCIONES.sql` | SQL Editor de TEST | si aborta -> leer el motivo, correr rollback |
| 2 | `ALTER ROLE ... LOGIN PASSWORD` (§3) | SQL Editor de TEST | -- |
| 3 | `B4_0A_v6_VERIFY.sql` | SQL Editor de TEST | **si hay 1 solo FAIL -> parar** |
...
## 2. Los siete gates de entrada (paso 1)

Los siete corren **antes** de crear un solo objeto de B4, en la misma transaccion.
Si cualquiera falla, **no se instala nada**.
...
| Recovery | **no vuelve a consumir la cuota LOGICA de la operacion.** Si paga cuota de **intentos** (ver §9.ter). |
| Dos admisiones **concurrentes** | **UN SOLO ticket** (una `nuevo`, otra `recovery`) y **una sola** cuota logica |
| Rechazado por **cuota** | **NO escribe el ledger** -> el nonce **no se quema** |
| Todo el ledger | **owner-only.** Cero grants de tabla (`G05`). |
| Integridad | `CHECK` de formato del hash (`G10`); coherencia con el ticket verificada (`G07`/`G08`/`G09`); **sin FK**, para no trabar la purga (`G11`). |

> **El claim correcto es "no vuelve a consumir la cuota LOGICA", no "no cobra ninguna
> cuota".** Decir lo segundo seria falso: el recovery **si** paga cuota de intentos.
> Sin eso, un replay masivo no tendria cota (ver §9.ter).
...
### Concurrencia -- serializacion por `(client_id, nonce)`

**El bug de v5.** El fast path leia el ledger **antes** de cobrar cuota. Dos
admisiones concurrentes del **mismo nonce** no se veian entre si (ninguna ve la fila
no commiteada de la otra): **las dos** pasaban el fast path, **las dos** cobraban la
cuota logica, y la perdedora devolvia `recovery` con sus cuotas **ya persistidas**
(estaban fuera del sub-savepoint). Resultado: **cuota cobrada dos veces por una sola
admision**. Medido: `cotizar_global = 2`, `cotizar_s2s = 2`.

**La solucion: advisory lock de transaccion**, derivado de `(client_id, nonce)`, que
se toma **antes** de leer el ledger:

```sql
PERFORM pg_advisory_xact_lock(hashtextextended(p_sujeto || '|' || p_nonce::TEXT, 0));
```

La segunda transaccion **espera** al commit de la primera y despues lee un ledger que
**ya tiene la fila** -> `recovery` limpio, **sin cuota logica**.

- Se libera solo al **COMMIT/ROLLBACK** (`xact`): no hay que soltarlo a mano.
- **MEDIDO: respeta `lock_timeout`** -> 55P03 -> lo captura el savepoint externo ->
  `{ok:false, error:'timeout'}` y **revierte todo, incluida la cuota de intentos**.
  "Sin mutaciones", como debe ser.
- Una colision de hash entre dos `(client_id, nonce)` distintos solo los serializa de
  mas: perdida de paralelismo, **nunca** de correccion.

El `UNIQUE` del ledger queda como **red de seguridad** (ahora envolviendo tambien las
cuotas logicas en el sub-savepoint, para que un `unique_violation` residual tampoco
las cobre). Con el advisory lock no deberia dispararse nunca.

---

## 9.ter. Dos capas de cuota -- por que el recovery no puede ser gratis

**El problema.** El recovery **no consume la cuota logica** de la operacion (esa es
su razon de ser). Pero si esa fuera **la unica** cuota, una misma solicitud valida se
podria repetir **indefinidamente** -- parse + SHA-256 + `SELECT` + conexion -- sin
tocar ningun limite. El replay masivo quedaba **sin cota**.

**La solucion: dos cubetas independientes.**

| Cubeta | Cuando se cobra | Limite/min |
|---|---|---|
| **`admision_intento_global`** | **toda** invocacion de `admitir` | 2400 |
| **`admision_intento_s2s`** | **toda** invocacion, por sujeto | 1200 |
| `cotizar/congelar/obtener_global` | **solo** al crear admision **nueva** | 600 / 120 / 1200 |
| `cotizar/congelar/obtener_s2s` | **solo** al crear admision **nueva** | 300 / 60 / 600 |

*(`admision_intento_web` = 240, para B4-0C.)*

**Dimensionado:** la cubeta de intentos va **por encima** de la suma de las logicas,
con margen para reintentos legitimos. Global: `600+120+1200 = 1920` -> **2400**.
S2S: `300+60+600 = 960` -> **1200**.

### Semantica final

| Caso | Cuota de intentos | Cuota logica |
|---|---|---|
| Admision **nueva** | **si** | **si** |
| **Recovery** | **si** | **no** |
| Binding distinto (`nonce_replay`) | **si** | **no** |
| **Replay masivo** | **lo frena** | -- |

La cuota de intentos se cobra **antes** del advisory lock: un flood se rechaza barato,
sin serializarse. Y las **cuatro** cubetas usan la **misma ventana** (`v_ventana` se
calcula una sola vez), asi que no se diluyen si el reloj cruza el minuto.
...
### Escenarios nuevos de v6 -- el bug de cuota que introdujo el ledger

| Escenario | Resultado |
|---|---|
| **Dos admisiones concurrentes** (mismo nonce): cuota **logica** | **+1, no +2** (era el bug) |
| Dos admisiones concurrentes: cuota de **intentos** | **+2** (las dos invocaciones pagan) |
| Dos admisiones concurrentes: tickets / ledger | **1 y 1** |
| 1 admision + **30 recoveries** del mismo nonce | intentos **31**, logica **1** |
| **Binding distinto** | paga intento, **no** paga logica |
| **Saturar `admision_intento_s2s`** (1200) | `rate_limited` / `cubeta: intento_sujeto`; **no** escribe ledger ni ticket; **no** llega a la logica |
| **Saturar `admision_intento_global`** (2400) | `rate_limited` / `cubeta: intento_global` |
| Advisory lock vs `lock_timeout` | **respeta** el timeout -> 55P03 -> `error: timeout`, **sin mutaciones** |
| Tras el timeout, la **MISMA** conexion | `SELECT 1` funciona (TXN1 **y** TXN2) |
| `R05` con error inesperado | **FAIL**, ya no "no evaluable = PASS" |
| `admision_leer` como el rol | **permission denied** |

### Escenarios de v5 (los dos bloqueantes anteriores), medidos en PostgreSQL real
...
```bash
python3 -m pip install -r B4_0A_v6_requirements.txt
python3 B4_0A_v6_HARNESS.py --reset   # PG + pg_cron(stub) + roles + stubs B3/B2A/B2B
python3 B4_0A_v6_SMOKES.py            # 250 PASS / 0 FAIL
```
...
| Suite | Resultado |
|---|---|
| `B4_0A_v6_SMOKES.py` | **250 PASS / 0 FAIL** |
| `B4_0A_v6_VERIFY.sql` | **69 PASS / 3 FAIL** -- los 3 FAIL son los fingerprints |
| `B4_0A_v6_ROLLBACK.sql` | **11 PASS / 3 FAIL** -- idem; residuo cero |
...
## 18. Manifest -- SHA-256 (B4-0A v6, 7 archivos)

```
04fb6144b5f63a51fc90ff5ada90676121f516f4bbe46ada39f0ac55bb437ae6  B4_0A_v6_ROL_TABLAS_FUNCIONES.sql
bc02eb129f247a369c32c8e62d6307d0861e75ee393afb685641e98ed442baa7  B4_0A_v6_VERIFY.sql
504baf91935341ba8a2e313d17b5c1c16970731ba76a14ab592ed0f83105e01d  B4_0A_v6_ROLLBACK.sql
(este archivo -- sha256sum B4_0A_v6_RUNSHEET.md)                  B4_0A_v6_RUNSHEET.md
046c57c5a3efd3fa6eb33152e108e69cc36e9dd09a057dde21d036f9cd15c216  B4_0A_v6_HARNESS.py
323b7d4213887a64471feefeff9e2a4d106765071d50021dc96dbb023bc87705  B4_0A_v6_SMOKES.py
21d09c3534126b937e95e7cac697e9671236767a48b98c159ed85ee2319e210d  B4_0A_v6_requirements.txt
```

**Nombres versionados `v6`.** Los `v5` quedan RETIRADOS: tenian el bug de doble
cobro de cuota logica en admisiones concurrentes. Si ves un archivo sin `v6` en el
nombre, es viejo y no sirve.
