-- ============================================================================
-- B4_0A_ROLLBACK.sql   (v4 -- post-auditoria 4)
--
...
DROP FUNCTION IF EXISTS precios_api.api_precios_nonce_consumir(TEXT, UUID);
...
INSERT INTO _r
SELECT 5, 'R05_sin_jobs_b4', '0', COALESCE(n::TEXT, '0 (sin pg_cron)'), COALESCE(n, 0) = 0
FROM (SELECT CASE
        WHEN EXISTS (SELECT 1 FROM pg_catalog.pg_extension WHERE extname = 'pg_cron')
