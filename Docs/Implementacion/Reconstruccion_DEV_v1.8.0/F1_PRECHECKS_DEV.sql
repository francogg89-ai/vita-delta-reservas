-- ============================================================================
-- RECONSTRUCCIÓN DE DEV — F1: PRECHECKS READ-ONLY
-- Canónico de referencia: 6B_SCHEMA_SQL.md v1.8.0 (Parte B + Parte C)
-- ----------------------------------------------------------------------------
-- USO: correr en el SQL Editor del PROYECTO DEV NUEVO, cada bloque por separado,
--      CON NADA SELECCIONADO (L-8A-01). El editor muestra solo el último SELECT,
--      por eso cada bloque cierra en una fila-veredicto.
--
-- NATURALEZA: 100% read-only. Solo leen catálogos del sistema. No crean, no
--      modifican, no consumen secuencias. Son seguros de correr en cualquier
--      proyecto: si por error se corren en OPS/TEST, P1 devuelve BASE_NO_VACIA.
--
-- GATE (corrección #2/#3): el discriminador de entorno NO es el ID de cabaña
--      (un bootstrap limpio nace 1-5 igual que TEST/OPS). El gate es:
--        (a) Project Ref correcto  -> confirmar por URL del navegador (P0)
--        (b) SQL Editor del proyecto nuevo
--        (c) base vacía            -> P1 == BASE_VACIA_OK
--      No habilitar F2 (Parte B) si P1 no da BASE_VACIA_OK.
-- ============================================================================
...
-- (L-7E-01). Confirmá el ref por la URL del navegador: debe ser el DEV NUEVO,
-- nunca lpiatqztudxiwdlcoasv (OPS).
-- ────────────────────────────────────────────────────────────────────────────
SELECT
...
-- 'ambiente'. TODO debe dar 0. El verdict se basa en los objetos NOMBRADOS del
...
SELECT
  ...
  CASE WHEN (SELECT n FROM base_tabs)    = 0
       ...
       THEN 'BASE_VACIA_OK -> habilita F2'
...
-- Corrección #6: si pg_cron_disponible es NULL -> pg_cron NO disponible en este
-- proyecto -> el Bloque 22 (cron.schedule) se omite/maneja, no se fuerza.
-- ────────────────────────────────────────────────────────────────────────────
SELECT
  split_part(version(), ' on ', 1) AS pg_version,
...
-- supabase_admin, authenticator). Importan para que el REVOKE de C12 y los
-- asserts de hardening de C14/F5 tengan sujeto real.
-- ────────────────────────────────────────────────────────────────────────────
...
--
-- LECTURA (corrección #4):
--  - OK (proyecto cerrado como OPS): 0 filas, o solo defaults inocuos del rol
--    postgres SIN privilegios de lectura/escritura a anon/authenticated/
--    service_role/PUBLIC.
--  - ALERTA (proyecto abierto, replicaría el residual A5 del DEV viejo): filas
--    que conceden SELECT/INSERT/UPDATE/DELETE a anon/authenticated/service_role.
--    Si aparece -> el switch no quedó OFF -> corregir en consola ANTES de F2.
--
