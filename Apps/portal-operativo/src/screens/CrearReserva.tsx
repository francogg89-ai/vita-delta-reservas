    ...
    <div className="mx-auto max-w-3xl space-y-4">
      ...
      {error && !estadoIncierto && (
        <Banner tono={error.code === 'conflicto' ? 'aviso' : 'error'}>
          {error.code === 'conflicto'
            ? 'Sin disponibilidad en ese rango (se solapa con una reserva, pre-reserva o bloqueo).'
      ...
      <div className="space-y-5 rounded-2xl border border-sand bg-white p-4 sm:p-6">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <Campo label="Check-in" requerido error={errores.fecha_in}>
            <input type="date" value={form.fecha_in} onChange={(e) => set('fecha_in', e.target.value)} className={controlClass} />
          </Campo>
          <Campo label="Check-out" requerido error={errores.fecha_out} hint="Noche de salida (no se cobra).">
            <input type="date" value={form.fecha_out} onChange={(e) => set('fecha_out', e.target.value)} className={controlClass} />
          </Campo>
        </div>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
