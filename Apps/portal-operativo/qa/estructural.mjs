...
// ---------------------------------------------------------------------------------------------
{
  ...
  //
  // Lo que se prueba aca, entonces, es el CLEANUP. La proteccion out-of-order en una MISMA
  // instancia se prueba en F7, y el token del contenedor en F1/F8.
  const p = await b.newPage({ viewport: { width: 1280, height: 900 } });
  await p.addInitScript(() => {
...
// ---------------------------------------------------------------------------------------------
caso('F7 · proteccion contra respuesta out-of-order en una misma instancia');
// ---------------------------------------------------------------------------------------------
{
  ...
  //
  // QUE PRUEBA Y QUE NO. F7 prueba el DESCARTE COMBINADO: con las dos guardas puestas (el codigo
  // real), la respuesta vieja no pisa a la nueva. NO aisla cual guarda lo logra -- eso lo hace
  // `npm run qa:mutacion`, que demuestra por SEPARADO que:
  //   - el cleanup solo (`!activo`) ya alcanza;
  //   - el reqId solo (`myId !== reqId.current`) ya alcanza;
  //   - sin ninguna de las dos, la vieja pisa (F2 gana).
  // Por eso este caso NO se rotula como prueba causal de `reqId`.
