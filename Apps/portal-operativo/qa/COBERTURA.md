# MATRIZ DE COBERTURA — SB-UI-6-FIX3
...
| protección out-of-order (descarte combinado) | **AUTO** — F7 prueba que con las dos guardas la vieja no pisa; el **mutation gate** demuestra por separado que cleanup solo alcanza, reqId solo alcanza, y sin ambos falla | `estructural` F7 · `qa:mutacion` |
...
| el bundle de probes no arrastra Supabase | **AUTO** — gate de pureza en `run-probes`: el metafile no puede contener `@supabase/supabase-js`, `src/lib/supabase.ts`, `createClient` ni `RealtimeClient` (si no, revienta en Node 20) |
